...
if [ -z $hap ] && [ -z $clbam ] && [ -z $comgvcf ];then
	echo "...Starting BaseRecalibrator & ApplyBQSR." >&1
	time seq 1 $threads | parallel --will-cite --joblog ${outdir}/temp.log \
	...
	exitval=`awk 'NR>1{if($7!=0){c=1; exit}}END{print c+0}' ${outdir}/temp.log`
	...
	seq 1 $threads | parallel --will-cite --joblog ${outdir}/temp.log \
...
if [ -z $clbam ] && [ -z $comgvcf ];then
	tmp_count=`find $outdir/bwa -maxdepth 1 -name "reads.prepped_*.bam" | wc -l`
	tmp_count2=`find $outdir/bwa -maxdepth 1 -name "reads.prepped_*.bai" | wc -l`
	...
	echo "...Calling haplotypes for $threads" >&1
...
# 04 CombineGVCFs
if [ -z $clbam ];then
	tmp_count=`find $outdir/gatk -maxdepth 1 -name "raw_*.g.vcf.gz" | wc -l`
	tmp_count2=`find $outdir/gatk -maxdepth 1 -name "raw_*.g.vcf.gz.tbi" | wc -l`
	
	([ $tmp_count -eq $tmp_count2 ]) || { echo >&2 ":( Index appear to be missing. Exiting!"; exit 1; }
	threads=$tmp_count
	echo "...Combining gVCFs for $threads" >&1
	arg=$(seq 1 $threads | parallel --will-cite -k "printf -- \" -V %s\" ${outdir}/gatk/raw_{}.g.vcf.gz")
