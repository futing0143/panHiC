...
for sample in $samples; do
    sample_gvcfs+=("-V" "$outdir/${sample}/gatk/${sample}.HC.g.vcf.gz")
done

if [ ! -e $outdir/population/${outname}.HC.vcf.gz ];then
...
time $gatk ApplyVQSR \
    ...
    -mode SNP \
	-truth-sensitivity-filter-level 99.0 && echo "** SNPs VQSR done **"
...
time $gatk ApplyVQSR \
    ...
    -mode INDEL \
    -truth-sensitivity-filter-level 95.0 && echo "** SNPs and Indels VQSR (${outname}.HC.VQSR.vcf.gz finish) done **"
