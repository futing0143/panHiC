...
def create_visualization(file, cooler_path, assembly, h3k27ac_path, RNA_path, loops_file):
    # 固定的基因列表
    try:
        gene_list = [line.strip() for line in open('/cluster/home/tmp/GBM/HiC/11SV/eaglec_new/result/EP_neoloop/all-genes.txt')]  # 读取基因列表
...
#        vis.plot_arcs(cutoff='top', gene_filter=gene_list, arc_color='#666666')
        ...
        output_folder = "/cluster/home/tmp/GBM/HiC/11SV/eaglec_new/result/EP_neoloop/allgene_CGClabel/pic"
        # 构建输出文件名，使用 file 和 neo 参数
        ...
        vis.outfig(output_file, dpi=300)
            ...
            # 如果遇到错误，记录文件名和错误信息
        with open("/cluster/home/tmp/GBM/HiC/11SV/eaglec_new/result/EP_neoloop/allgene_CGClabel/error_log.txt", "a") as log_file:
...
def main(filelist_path):
    ...
    for index, row in df.iterrows():
        ...
        create_visualization(file, cooler_path, assembly, h3k27ac_path, RNA_path, loops_file)
