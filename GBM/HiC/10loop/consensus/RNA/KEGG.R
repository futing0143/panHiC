...
tcga_rna=read.csv('/cluster/home/futing/Project/GBM/HiC/13mutation/gbm_tcga_gdc/data_mrna_seq_tpm.txt',sep='\t')
trans1 <- bitr(tcga_rna$Entrez_Gene_Id, fromType="ENTREZID",toType="SYMBOL", OrgDb="org.Hs.eg.db")
tcga_rna=merge(tcga_rna,trans1,by.x='Entrez_Gene_Id',by.y='ENTREZID',all=T)
qc=merge(qc,trans1,by.x='biomart',by.y='SYMBOL',all=T)
write.table(tcga_rna,'/cluster/home/futing/Project/GBM/HiC/13mutation/mutation_tcga/RNA.txt',sep='\t')
write.table(qc,'/cluster/home/futing/ref_genome/hg38_gencode/genebed/geneid.txt',sep='\t')
