...
FNR > 1 {
    ...
    for (i = 4; i <= last_col; i++) {
        for (j = i + 1; j <= last_col; j++) {
            key = col_names[i] "_" col_names[j]
            if ($i != "0" && $i != "NA" && $j != "0" && $j != "NA") {
                intersection[key]++
                union[key]++
            } else if (($i != "0" && $i != "NA") || ($j != "0" && $j != "NA")) {
                union[key]++
            }
        }
    }
...
function print_results() {
    ...
    for (i = 1; i <= length(combos); i++) {
        ratio = (combos[i] in union && union[combos[i]] > 0) ? (intersection[combos[i]] / union[combos[i]]) : 0
        printf " %.4f", ratio >> output_file
    }
    printf "\n" >> output_file
}
' "$@"
