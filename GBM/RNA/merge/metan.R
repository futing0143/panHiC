...
merged_data =data
merged_data$ensembl_gene_id=rownames(merged_data)
merged_data=merge(merged_data,qc[,c(1,4)],by='ensembl_gene_id')
merged_data=merged_data[,-1]
# 转换数据类型，这个merged_data里面是字符串前有个空格(trimws去掉)
merged_data[,1:(ncol(merged_data)-1)] <- as.data.frame(lapply(merged_data[,1:(ncol(merged_data)-1)], 
                                                              function(x) as.numeric(trimws(x))))

# 合并基因
merged_data1= aggregate(x=merged_data[,1:(ncol(merged_data)-1)],
                        by=list(gene_id=merged_data$raw),
                        FUN=sum)%>%
