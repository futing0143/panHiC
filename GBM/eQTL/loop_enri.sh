...
##04 用snp注释loop
awk 'NR > 1 {print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$1"_"$2"_"$3"_"$4"_"$5"_"$6"\t"$7"\t"$8"\t"$9"\t"$10}' /cluster/home/tmp/GBM/HiC/10loop/consensus/mid/GBM/GBM_merged.bedpe >GBM-peakachu-10kb-loops.0.95_label.bedpe
pairToBed -a GBM-peakachu-10kb-loops.0.95_label.bedpe -b /cluster/home/tmp/gaorx/GBM/eQTL/anno/snp_gene_hg38.bed >  loop_snp.bedpe ##either- Report overlaps if either end of A overlaps B.
