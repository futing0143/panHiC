...
if [ "$first_stage" == "genotype" ]; then
	...
	#--annotation QD --annotation FS --annotation MQ --annotation MQRankSum --annotation ReadPosRankSum --annotation SOR --annotation DP  \--disable-read-filter MappingQualityReadFilter --dont-use-soft-clipped-bases false
	seq 1 $threads | parallel --will-cite --joblog temp.log "gatk  --java-options \"-Xmx4G -XX:+UseParallelGC -XX:ParallelGCThreads=1\" HaplotypeCaller \
		...
		--disable-read-filter MappingQualityReadFilter --smith-waterman FASTEST_AVAILABLE \
		--min-base-quality-score $mbq --emit-ref-confidence GVCF"
	exitval=`awk 'NR>1{if($7!=0){c=1; exit}}END{print c+0}' temp.log`
	...
	gatk --java-options "-Xmx4G" CombineGVCFs -R $reference -V $arg -O raw.vcf
