...
setwd("/cluster/home/futing/Project/GBM/legency/gsc/h3k27ac")
...
#读入文件 ----
meta_chip=read_excel("meta_chip.xlsx")
write.csv(meta_chip, "meta_chip.csv", row.names = FALSE,quote = F)

# --- 01 创建 DiffBind 对象进行PCA分析 ----
###确保文件都有值，不然会报错
dbObj <- dba(sampleSheet = "/cluster/home/futing/Project/GBM/legency/gsc/h3k27ac/meta_chip.csv")
...
write.table(meta_chip_match, "meta_chip_match.txt",sep='\t',row.names = FALSE,quote = F)

# --- 01 创建 DiffBind 对象进行PCA分析 ----
###确保文件都有值，不然会报错
dbObj_match <- dba(sampleSheet = "meta_chip_match.txt")
###存储的是原始peaks数据，没有通过reads数进行校正
