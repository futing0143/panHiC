...
EOF
    ...
    if [ ! -f ${dir}/cool/${cell}_1000.cool ];then 
        ...
        jid_mustache=$(submit_job "${cell}_mustache" "${scripts}/mustache_single.sh ${dir}" "$jid_dots2")
        echo "${cell}_mustache Job ID: $jid_mustache"
        
        jid_fithic=$(submit_job "${cell}_fithic" "${scripts}/fithic_single.sh ${dir}" "$jid_dots2")
