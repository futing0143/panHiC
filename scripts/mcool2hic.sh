...
if [[ $input_mcool == *.mcool ]]; then
    ...
    output_bedpe="${input_mcool%.cool}.${resolution}.bedpe"
    echo -e "cooler dump --join -r $highest_res $input_mcool::/resolutions/$highest_res -> $output_bedpe"
    ...
    java -Xmx200G -jar $juicer_tools_jar pre --threads 20 -j 20 $output_bedpe $output_hic $chrom_sizes
    ...
    output_bedpe="${input_mcool%.cool}.${resolution}.bedpe"
    echo -e "cooler dump --join -r ${resolution} $input_mcool -> $output_bedpe"
    ...
    java -Xmx400G -jar $juicer_tools_jar pre --threads 20 -j 20 $output_bedpe $output_hic $chrom_sizes
fi
