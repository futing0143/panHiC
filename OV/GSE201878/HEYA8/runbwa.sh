...
#SBATCH --output=/cluster2/home/futing/Project/panCancer/OV/GSE201878/HEYA8/bwa-%j.log
#SBATCH -J "HEYA8"

wkdir=/cluster2/home/futing/Project/panCancer/OV/GSE201878/HEYA8
cd ${wkdir}
...
cat ${wkdir}/srr.txt | while read -r srr;do
...
while read -r name;do
    ...
    sort -T "${tmpdir}" --parallel=20 -k2,2d -k6,6d -k4,4n -k8,8n -k1,1n -k5,5n -k3,3n "${name}${ext}.frag.txt" > "${name}${ext}.sort.txt"
    if [ $? -ne 0 ]; then
...
sh /cluster2/home/futing/Project/panCancer/OV/sbatch.sh GSE201878 HEYA8 HindIII "-S merge"
