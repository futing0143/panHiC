...
def read_files(bedpe_path: str, gene_bed_path: str, enhancer_bed_path: str,
    ...
    loops_df = pd.read_csv(
        bedpe_path, sep='\t', header=None, usecols=[0,1,2,3,4,5]
    )
...
def get_anchor_info(features):
    if not features:
        return "NA"
    return "|".join(f['full'] for f in features)


# =========================================================
# Core loop processing (FAST)
# =========================================================

def process_loops_fast(loops_df, genes_df, enhancers_df, flanking_size=5000):
    ...
    for loop in loops_df.itertuples(index=False):
        ...
        g1 = find_overlaps_fast(
            genes_by_chr.get(loop.chr1), loop.start1, loop.end1, flanking_size
        )
        e1 = find_overlaps_fast(
            enh_by_chr.get(loop.chr1), loop.start1, loop.end1, flanking_size
        )
        g2 = find_overlaps_fast(
            genes_by_chr.get(loop.chr2), loop.start2, loop.end2, flanking_size
        )
        e2 = find_overlaps_fast(
            enh_by_chr.get(loop.chr2), loop.start2, loop.end2, flanking_size
        )
...
def process_single_sample(cancer, gse, cell, atac_path, gene_bed_path,
    ...
    if sum(1 for _ in open(bedpe)) <= 1000:
        return 'skipped'
    ...
    loops, genes, enh = read_files(
        bedpe, gene_bed_path, enhancer,
        enhancer == public_db
    )

    res = process_loops_fast(loops, genes, enh, flank)

    out = f"{output_dir}/{cancer}/{gse}/{cell}/anno/mustache/{cell}_loop_annotation.tsv"
    os.makedirs(os.path.dirname(out), exist_ok=True)
    res['enhancer_source'] = 'public_db' if enhancer == public_db else 'cancer_specific'
...
def main():
    ...
    ap.add_argument('--atac-bed', required=False,default='/cluster2/home/futing/Project/panCancer/Analysis/anno_loops/ATAC_bedlist.txt')
