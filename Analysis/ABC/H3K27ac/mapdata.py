...
def main():
    ...
    # 2. 读取四个数据源的txt文件
    GEO_file = '/cluster2/home/futing/Project/panCancer/Analysis/ABC/H3K27ac/GEO/GEO.txt'
    ENCODE_file = '/cluster2/home/futing/Project/panCancer/Analysis/ABC/H3K27ac/ENCODE/ENCODE_cell.txt' 
    cistrome_file = '/cluster2/home/futing/Project/panCancer/Analysis/ABC/ATAC/cistrome/H3K27ac/cistrome.txt'
    ...
    for idx, row in meta_df.iterrows():
        ...
        # 汇总：至少有一个数据源存在该样本
        has_any = 1 if (has_geo or has_encode or has_cistrome) else 0
    ...
    result_df['has_any_H3K27ac'] = has_any_list
    
    # 5. 保存结果
    output_file = 'meta_with_H3K27ac_matches.txt'
    ...
    print(f"至少有一种H3K27ac数据的行数: {result_df['has_any_H3K27ac'].sum()}")
    print(f"3种H3K27ac数据都有的行数: {((result_df['GEO'] == 1) & (result_df['ENCODE'] == 1) & (result_df['Cistrome'] == 1)).sum()}")
    
    # 按唯一样本统计
    print("\n" + "="*60)
    print("按唯一样本统计:")
    print("="*60)
    unique_stats = result_df.groupby('clcell')[['GEO', 'ENCODE', 'Cistrome', 'has_any_H3K27ac']].max()
    ...
    print(f"至少有一种H3K27ac数据的唯一样本数: {unique_stats['has_any_H3K27ac'].sum()}")
