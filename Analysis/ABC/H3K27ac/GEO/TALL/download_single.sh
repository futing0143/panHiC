#!/bin/bash

debugdir="/cluster2/home/futing/Project/panCancer/Analysis/ABC/H3K27ac/GEO/TALL"
cd $debugdir


mkdir -p "${debugdir}/debug"
submit_job() {
    local name=$1
sbatch <<- EOF | egrep -o -e "\b[0-9]+$"
#!/bin/bash -l
#SBATCH -p gpu
#SBATCH -t "5780"
#SBATCH --cpus-per-task=10
#SBATCH --output=$debugdir/debug/${name}_pair-%j.log
#SBATCH -J "${name}_pair"



date
source activate RNA
cd $debugdir
export TMPDIR=${debugdir}/debug
echo -e "parallel-fastq-dump --sra-id ${name} --threads 40 --outdir ./ --split-3 --gzip"
parallel-fastq-dump --sra-id ./${name} --threads 40 --outdir ./ --split-3 --gzip
date
EOF
}

IFS=$','
# while read -r name other;do
#     source activate RNA
# 	echo "Processing SRR: ${name}"
# 	echo $name > tmp
# 	# prefetch -p -X 100GB ${name}
# 	/cluster/home/futing/pipeline/Ascp/ascp2.sh tmp ./ 40M
# 	if [ -s ${name} ];then
# 	# prefetch -p -X 60GB ${name}
# 		jid=$(submit_job "${name}")
# 	fi
# done < <(tail -n +2 SraRunTable.csv | cut -f1 -d ',')

for name in SRR8939294 SRR8939308 SRR8939314;do
	echo "Processing SRR: ${name}"
	echo $name > tmp
	# prefetch -p -X 100GB ${name}
	/cluster/home/futing/pipeline/Ascp/ascp2.sh tmp ./ 40M
	if [ -s ${name} ];then
	# prefetch -p -X 60GB ${name}
		jid=$(submit_job "${name}")
	fi
done 