...
# -- method2 
com1 <- prcomp(df_filtered , center = T,scale = TRUE)

# 02 提取PC score并确定分组###
pc1_pca<-com1$x %>% as.data.frame(.) %>% 
  rownames_to_column(var = "ncell") 

pc1_pca =merge(pc1_pca,meta,by.x='ncell',by.y='header')
pc1_pca$cancer <-factor(pc1_pca$cancer)
pc1_pca$Datasource <-factor(pc1_pca$Datasource)
...
gradient_colors <- colorRampPalette(set1)(39) #%>% rev()
...
p2
ggsave("./TPM_Cancer_PCA1203.pdf", egg::set_panel_size(p2, width=unit(3, "in"), height=unit(3, "in")), 
       width = 8, height = 6, units = 'in')

#-------------- 05 kmeans
set.seed(123) 
pca_result = pc1_pca
kmeans_result <- kmeans(pca_result[2:22],center=3)
...
pcasource <-
  ggplot(data = pca_result,aes(x=PC1,y=PC2,fill=Datasource))+
  #散点图
  #stat_ellipse(aes(fill=dataset),type = "norm", geom ="polygon",alpha=0.2,color=NA)+
  geom_point(stroke=0.35,shape=21,size=2,color="black",alpha=0.8)+
  scale_color_brewer(palette = 'Set1')+
...
pcacancer_legend <- get_legend(
  ggplot(data = pca_result,aes(x=PC1,y=PC2,fill=cancer)) +
    ...
    theme(legend.position = "right",
          ...
          legend.background = element_rect(fill =NA, colour = NA),
          legend.margin = margin(t = 0, r = 0, b = 0, l = 0, unit = "pt")))
pcasource_legend <- get_legend(
  ggplot(data = pc1_pca,aes(x=PC1,y=PC2,fill=Datasource))+
...
# 最终组合
grid.arrange(p2,pcasource,pcakmeans,
  legend_block,
  layout_matrix = lay,
