...
class PlotBigWig(TrackPlot):
	...
	def plot(self, ax, chrom_region, start_region, end_region):
		...
		# print(f"x_values length: {len(x_values)}")  # 调试输出
	...
	def __get_scores_per_bin(self, genome_range, num_bins, max_try_nums=5):
		...
		while num_tries < max_try_nums:
			...
			try:
				...
				print(f"Raw scores_per_bin length: {len(scores_per_bin)}")  # 调试输出
				# 如果长度不匹配，调整到 num_bins
				if len(scores_per_bin) != num_bins:
					x_old = np.linspace(0, 1, len(scores_per_bin))
					x_new = np.linspace(0, 1, num_bins)
					scores_per_bin = np.interp(x_new, x_old, scores_per_bin)
					print(f"Adjusted scores_per_bin length: {len(scores_per_bin)}")  # 调试输出
				...
				break
		# print(f"Final scores_per_bin length: {len(scores_per_bin)}")  # 调试输出
...
class SigTrack(object):
	...
	def stats(self, chrom, start, end, nBins):
		...
		'''
		# Initialize output array, length = nBins
		arr = np.zeros(nBins)
		
		for si, ei, c, s, e, o in self.blocks:
			# Calculate target bins for each block
			target_bins = min(ei - si, nBins - si)  # Ensure we don't exceed the array length
			
			if target_bins <= 0:
				continue  # Skip if the target bins is zero or negative
				
			# Get data from bigWig file
			part = np.array(self.bw.stats(c, s, e, nBins=target_bins)).astype(float)
			
			# If part length doesn't match target_bins, adjust it
			if len(part) != target_bins:
				x_old = np.linspace(0, 1, len(part))
				x_new = np.linspace(0, 1, target_bins)
				part = np.interp(x_new, x_old, part)
				
			# Make sure the slice we're assigning to is within bounds
			if si >= nBins:
				continue
				
			end_idx = min(ei, nBins)
			
			# Ensure part length matches the slice length
			if len(part) != (end_idx - si):
				part = part[:end_idx - si]
				
			if o == '+':
				arr[si:end_idx] = part
			else:
				arr[si:end_idx] = part[::-1]
		
		arr = arr * self.multiply
		print(f"SigTrack.stats output length: {len(arr)}")  # Debug output
		return arr
